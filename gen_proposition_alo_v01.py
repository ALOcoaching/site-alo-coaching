#!/usr/bin/env python3
"""
Genere la proposition commerciale ALO Coaching V01 -- Design ameliore.
Mise a jour avec perimetre valide -- Avril 2026.
...
LOGO_PATH = os.path.join(BASE_DIR, "img", "logo-alo.png")
LOGO_NOIR_PATH = os.path.join(BASE_DIR, "img", "logo-alo-noir.png")
...
def add_footer(slide, text="ALO Coaching & Formation  |  Arnaud Loyet  |  Proposition commerciale – Avril 2026"):
...
add_text_box(sl1, Inches(1.2), Inches(2.85), Inches(9), Inches(0.9),
             "Direction de Projets & Management du Site de Lannion — V01",
...
tb_author = add_text_box(sl1, Inches(1.2), Inches(4.9), Inches(9), Inches(0.8),
             "Arnaud Loyet  —  Avril 2026", font_size=15, color=RGBColor(0x77, 0x77, 0x77),
             font_name="Calibri")
...
add_header_bar(sl_som, "SOMMAIRE", "Proposition commerciale – Avril 2026")
add_logo_noir(sl_som)

# Sommaire items — visual table of contents
som_items = [
    ("01", "Périmètre juin", "8 jours (L+M) + 2 jours formation HSK2 à Rennes · 10 jours"),
    ("02", "Périmètre juillet / août", "3 jours fin juillet + 3 jours fin août · 6 jours"),
    ("03", "Périmètre septembre → décembre", "Option 1 validée (2j/sem) · Option 2 en option"),
...
add_header_bar(sl2, "01  ·  PÉRIMÈTRE — JUIN",
               "À partir du 8 juin  ·  Lundis & mardis + Formation HSK2  ·  10 jours  ·  Basé site de Lannion")
add_logo_noir(sl2)
...
add_rounded_shape(sl2, col1_left + Inches(0.03), col1_top + Inches(0.03),
                  col1_w, Inches(3.2), fill_color=RGBColor(0xE0, 0xDA, 0xD0))
# Main card
card1 = add_rounded_shape(sl2, col1_left, col1_top, col1_w, Inches(3.2),
...
# Gold accent
add_gold_accent(sl2, col1_left, col1_top + Inches(0.6), Inches(2.6))
...
# Volume badge
add_rounded_shape(sl2, col1_left + Inches(0.2), col1_top + Inches(2.65), Inches(2.8), Inches(0.45),
                  fill_color=NAVY)
add_text_box(sl2, col1_left + Inches(0.2), col1_top + Inches(2.65), Inches(2.8), Inches(0.45),
...
add_rounded_shape(sl2, col2_left + Inches(0.03), col1_top + Inches(0.03),
                  col2_w, Inches(3.2), fill_color=RGBColor(0xE0, 0xDA, 0xD0))
# Card
card2 = add_rounded_shape(sl2, col2_left, col1_top, col2_w, Inches(3.2),
                          fill_color=CARD_BG, line_color=SUBTLE_BORDER, line_width=Pt(0.75))
...
add_gold_accent(sl2, col2_left, col1_top + Inches(0.6), Inches(2.6))
...
# Volume badge
add_rounded_shape(sl2, col2_left + Inches(0.2), col1_top + Inches(2.65), Inches(2.8), Inches(0.45),
                  fill_color=NAVY)
add_text_box(sl2, col2_left + Inches(0.2), col1_top + Inches(2.65), Inches(2.8), Inches(0.45),
             "≈ 1.5 jours / semaine", font_size=13, color=GOLD, bold=True,
             alignment=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)

# ── Formation HSK2 card (full width) ──
form_top = Inches(5.0)
form_left = Inches(0.7)
form_w = SLIDE_W - Inches(1.4)
form_h = Inches(0.95)

add_rounded_shape(sl2, form_left + Inches(0.03), form_top + Inches(0.03),
                  form_w, form_h, fill_color=RGBColor(0xE0, 0xDA, 0xD0))
add_rounded_shape(sl2, form_left, form_top, form_w, form_h,
                  fill_color=CARD_BG, line_color=SUBTLE_BORDER, line_width=Pt(0.75))
add_gold_accent(sl2, form_left, form_top, form_h)

add_text_box(sl2, form_left + Inches(0.25), form_top + Inches(0.08),
             Inches(5), Inches(0.35),
             "🎓  FORMATION HSK2 / PILOTER SON PÉRIMÈTRE",
             font_size=16, color=NAVY, bold=True)
add_text_box(sl2, form_left + Inches(0.25), form_top + Inches(0.45),
             Inches(8), Inches(0.4),
             "10 et 11 juin  ·  2 jours  ·  À Rennes  ·  Avec frais de déplacement (conditions OPEN)",
             font_size=13, color=MEDIUM_TEXT)

add_rounded_shape(sl2, form_left + form_w - Inches(2.2), form_top + Inches(0.2),
                  Inches(1.8), Inches(0.5), fill_color=NAVY)
add_text_box(sl2, form_left + form_w - Inches(2.2), form_top + Inches(0.2),
             Inches(1.8), Inches(0.5),
             "+ 2 jours", font_size=14, color=GOLD, bold=True,
             alignment=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)

# Bottom summary bar
add_rounded_shape(sl2, Inches(0.7), Inches(6.2), SLIDE_W - Inches(1.4), Inches(0.55),
                  fill_color=NAVY)
add_text_box(sl2, Inches(0.7), Inches(6.2), SLIDE_W - Inches(1.4), Inches(0.55),
             "TOTAL JUIN  ·  10 jours  ·  8 jours (L+M à partir du 8 juin) + 2 jours formation HSK2 (Rennes)",
...
add_header_bar(sl3, "02  ·  PÉRIMÈTRE — JUILLET / AOÛT",
               "Interventions de clôture  ·  3 jours fin juillet + 3 jours fin août  ·  6 jours")
add_logo_noir(sl3)
...
add_paragraph(tf3, "📅  Clôture fin juillet", font_size=19, color=NAVY, bold=True,
              space_before=Pt(4))
add_paragraph(tf3, "     29, 30 et 31 juillet 2026  —  3 jours", font_size=14,
...
add_paragraph(tf3, "📅  Clôture fin août", font_size=19, color=NAVY, bold=True,
              space_before=Pt(8))
add_paragraph(tf3, "     27, 28 et 31 août 2026  —  3 jours", font_size=14,
              color=MEDIUM_TEXT, space_before=Pt(6))
...
add_header_bar(sl4, "03  ·  PÉRIMÈTRE — SEPTEMBRE À DÉCEMBRE",
               "Option 1 validée  ·  Option 2 en option  ·  4 mois  ·  Basé site de Lannion")
add_logo_noir(sl4)
...
# Header bar — gold to show validation
add_shape(sl4, opt1_left, opt1_top, opt1_w, Inches(0.55), fill_color=GOLD)
add_text_box(sl4, opt1_left, opt1_top, opt1_w, Inches(0.55),
             "OPTION 1  —  Site / DCDP + DP Orange  ✅ VALIDÉ", font_size=16, color=NAVY, bold=True,
             alignment=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
...
# Header bar — navy for option
add_shape(sl4, opt2_left, opt1_top, opt2_w, Inches(0.55), fill_color=NAVY)
add_text_box(sl4, opt2_left, opt1_top, opt2_w, Inches(0.55),
             "OPTION 2  —  Focus DP Orange  (en option)", font_size=16, color=RGBColor(0xAA, 0xAA, 0xAA), bold=True,
             alignment=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
...
add_text_box(sl4, Inches(0.7), Inches(6.75), (SLIDE_W - Inches(1.4)) / 2 - Inches(0.1), Inches(0.45),
             "Option 1 : 32 jours (sept→déc)  ✅ VALIDÉ", font_size=14, color=NAVY, bold=True,
             alignment=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
...
add_text_box(sl4, Inches(0.7) + (SLIDE_W - Inches(1.4)) / 2 + Inches(0.1), Inches(6.75),
             (SLIDE_W - Inches(1.4)) / 2 - Inches(0.1), Inches(0.45),
             "Option 2 : 24 jours (sept→déc)  — en option", font_size=14, color=RGBColor(0xAA, 0xAA, 0xAA),
             alignment=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
...
add_header_bar(sl5, "04  ·  VOLET FINANCIER",
               "TJM 800 € HT  ·  Portage via Jump  ·  Option 1 validée")
add_logo_noir(sl5)
...
data = [
    ("Juin (8j L+M + 2j formation HSK2)", "10 j", "8 000 €", "+ 150 €", "8 150 €"),
    ("Juillet (29, 30, 31 juil.)", "3 j", "2 400 €", "+ 150 €", "2 550 €"),
    ("Août (27, 28, 31 août)", "3 j", "2 400 €", "+ 150 €", "2 550 €"),
    ("Sept→Déc · Option 1 ✅ VALIDÉ (2j/sem)", "32 j", "25 600 €", "+ 600 €", "26 200 €"),
    ("Sept→Déc · Option 2 (en option)", "24 j", "19 200 €", "+ 600 €", "19 800 €"),
]
...
# ── Totaux annuels ──
total_top = table_top + row_h * 6 + Inches(0.2)
total_w = Inches(3.8)
total_h = Inches(1.0)

for i, (label, val) in enumerate([
    ("TOTAL ANNUEL · Opt 1 + Jump  ✅", "Juin + été + 2j/sem\n39 450 € HT"),
    ("TOTAL ANNUEL · Opt 2 + Jump", "Juin + été + focus Orange\n33 050 € HT"),
]):
...
cond_items = [
    ("📋  Validité de la proposition",
     "La présente proposition est valable 30 jours   à compter de sa date d’émission (avril 2026)."),
    ("💰  Facturation",
...
data7 = [
    ("Juin (8j L+M + 2j formation HSK2)", "10 j", "8 000 €", "+ 400 €", "8 400 €"),
    ("Juillet (29, 30, 31 juil.)", "3 j", "2 400 €", "+ 120 €", "2 520 €"),
    ("Août (27, 28, 31 août)", "3 j", "2 400 €", "+ 120 €", "2 520 €"),
    ("Sept→Déc · Option 1 (2j/sem)", "32 j", "25 600 €", "+ 1 280 €", "26 880 €"),
    ("Sept→Déc · Option 2 (en option)", "24 j", "19 200 €", "+ 960 €", "20 160 €"),
]
...
# ── Totaux ITG ──
tot7_top = table7_top + row7_h * 6 + Inches(0.15)
tot7_w = Inches(3.5)
tot7_h = Inches(0.85)

for i, (label, val) in enumerate([
    ("TOTAL ANNUEL · Opt 1 + ITG", "40 320 € HT"),
    ("TOTAL ANNUEL · Opt 2 + ITG", "33 600 € HT"),
]):
...
add_footer(sl7, text="ALO Coaching & Formation  |  Arnaud Loyet  |  Annexe – Avril 2026")


# ═══════════════════════════════════════════════════════════════
# Sauvegarde
# ═══════════════════════════════════════════════════════════════
output_path = os.path.join(BASE_DIR, "Proposition_ALO_Coaching_OPEN_2026_V01.pptx")
